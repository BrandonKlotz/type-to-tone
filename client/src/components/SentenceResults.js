...
class SentenceResults extends Component {
            ...
            sentence => {
                const classnames = join(
                    sentence.tones.map(tone => {
                        return `${tone.tone_name === this.state.highlighted? " highlighted " + tone.tone_id: ""}`;
                    })," "
                );
    ...
    mapSentencesAndReturnEmotionsArray = sentences => {
        var detectedEmotionsArray = sentences.map(sentence => {
            return this.mapIndividualSentenceTones(sentence);
        });
    ...
    mapIndividualSentenceTones = sentence => {
        var individualEmotionsArray = sentence.tones.map(tone => {
